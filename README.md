...
**Pascal** binding for [libpolycall](https://github.com/obinexus/libpolycall) — a
scaffolded adapter (TODO tracked).

This binding is a *thin adapter*: it contains no runtime or config logic. It
maps Pascal idioms onto the shared C core through the FFI boundary
(`include/polycall/polycall.h`, `include/polycall/polycall_ffi.h`). See
[../../docs/adapter-pattern.md](../../docs/adapter-pattern.md).

## Config

Read-only config: [`pascal-polycallrc`](pascal-polycallrc) — the standard `*polycallrc` convention,
